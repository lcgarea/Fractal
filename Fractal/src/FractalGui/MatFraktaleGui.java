...
public class MatFraktaleGui {
   ...
   public MatFraktaleGui(StartFenster startFenster){
    this.startFenster = startFenster;
   }

    public void go(){
        frame = new JFrame("Mathematische Fraktale");
       ...
       fractalChooser = new JComboBox<>(
        ...
        .filter(f -> !(f.getFractalPanel() instanceof GeoFraktalBase))
       ...
       //Layout
        ...
        for(FractalType fractalType : FractalType.values()){
            if(!(fractalType.getFractalPanel() instanceof GeoFraktalBase)){
    ...
    private void update(Consumer<MatFraktalBase> action){
        ...
        if (currentFractalPanel instanceof MatFraktalBase){
            MatFraktalBase fractal = (MatFraktalBase) currentFractalPanel;
